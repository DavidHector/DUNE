...
/*************************************/
/* This example solves surface flow on an inclined plane.
 */
/*************************************/
...
public:
  ...
  Number bathymmetry (const E& e, const X& xlocal) const
  {
    ...
    // return 0.01*x[0]+0.1*std::abs(x[1]-50.0);
    return 5.0+0.01*x[0]+0.04*x[0];
  }
  ...
  Number g (const E& e, const X& x) const
  {
    ...
    Number z = 2.0*std::sin(M_PI*0.5*std::max(0.0,1.0-(r/rmax)))+bathymmetry(e,x);
    return z;
  }
...
int main(int argc, char** argv)
{
  try{
    ...
    ptreeparser.readINITree("secondexample.ini",ptree);
