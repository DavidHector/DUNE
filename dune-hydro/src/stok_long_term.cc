...
class Model
{
  ...
  Number eps,A,B;
  double time;
  std::vector<RasterDataSet<short>> ga;
  ...
  {
    // glacierized area: one data file per year
    std::cout << "=====> READING GLACIERISED AREA DATA for " << year << std::endl;
    std::string combinedname = path_to_data + "Glacierised_Area/";
    {
      char filename[512];
      sprintf(filename,"Glac_%4d.tif",year);
      GeoTIFFImage<GByte> image(filename,combinedname,1,2);
      ga.push_back(RasterDataSet<short>(ox+0.5*dx,oy+0.5*dy,dx,dy,N[0],N[1],0,1));
      ga.back().paste(image);
    }
    std::cout << ga.size() << " glacierised area files read successfully" << std::endl;
    
    // temperature
    std::cout << "=====> READING TEMPERATURE DATA for " << year << std::endl;
    combinedname = path_to_data + "TEMPERATURE/TEMP_" + std::to_string(year) + "/";
    for (int day=1; day<=days; day++)
      {
        char filename[512];
        sprintf(filename,"%4d%03d.tif",year,day);
    ...
    combinedname = path_to_data + "PRECIPITATION/PRECP_" + std::to_string(year) + "/";
    for (int day=1; day<=days; day++)
      {
        char filename[512];
        sprintf(filename,"%4d%03d.tif",year,day);
    ...
    std::cout << precip.size() << " precipitation files read successfully" << std::endl;

    // snow cover data
    std::cout << "=====> READING SNOW COVER DATA for " << year << std::endl;
    combinedname = path_to_data + "SCA_2002-2019_FINAL/SCA_" + std::to_string(year) + "/";
    for (int day=1; day<=days; day++)
      {
        char filename[512];
        sprintf(filename,"MOYD10A1GL06_Maximum_Snow_Extent_%4d%03d.tif",year,day);
        GeoTIFFImage<short> image(filename,combinedname,1,2);
        sc.push_back(RasterDataSet<short>(ox+0.5*dx,oy+0.5*dy,dx,dy,N[0],N[1],0,1));
        sc.back().paste(image);
      }
    std::cout << sc.size() << " snow cover files read successfully" << std::endl;

    years_available++;
    days_available += days;
    first_year = std::min(first_year,year);
  }
  
public:
  //! Constructor gets parameter tree to read ini file parameters
  Model (Dune::ParameterTree ptree_)
    : ptree(ptree_), eps(1e-2), days_available(0), years_available(0), first_year(100000)
  {
    ...
    for (std::size_t i=0; i<gwdepth_raster.data().size(); i++)
      {
        ...
        // In this version the glacierized area varies over time and thus the groundwater depth
        // would also depend on time.
        // we do this differently: Just set the infiltration coefficient to zero under the glacier
        // if (ga.data()[i]==200)
        //   gwdepth_raster.data()[i] = 0.5; // under the glacier pixel
      }

    // add data set for one year
    for (int year=2003; year<=2019; year++) add_data(year,365);
  ...
  const RasterDataSet<short>& glacier (std::size_t i)
  {
    i = std::min(i,ga.size()-1);
    return ga[i];
  }
  ...
  Number f (const E& e, const X& xlocal) const
  {
    ...
    int cellx = std::floor(x[0]/H[0]);
    int celly = std::floor(x[1]/H[1]);
    int day = day_from_time(time);
    day = std::min(day,(int)sc.size()-1);
    day = std::min(day,(int)temp.size()-1);
    int year = year_from_day(day);
    year = std::min(year,(int)ga.size()-1);
    
    auto snowcover_value = sc[day](cellx,celly);
    auto temperature_value = temp[day](cellx,celly);
    auto precipitation_value = precip[day](cellx,celly) * mm_per_day_to_meter_per_second;
    auto glacier_value = ga[year](cellx,celly);
  ...
  Number q (const E& e, const X& xlocal, Number u_surface, Number u_groundwater, Number bath_surf) const
  {
    ...
    // no exchange with rock 
    if (gwdepth_raster(cellx,celly)<1.0)
      return 0.0;

    // no exchange under glacier
    int day = day_from_time(time);
    int year = year_from_day(day);
    year = std::min(year,(int)ga.size()-1);
    auto glacier_value = ga[year](cellx,celly);
    if (glacier_value==200)
      return 0.0;
...
void driverFVNewNewton (Dune::ParameterTree& ptree)
{
  ...
  for (size_t i=0; i<model.glacier(0).data().size(); i++)
    Dune::PDELab::Backend::native(glacier_vec)[i][0] = model.glacier(0).data()[i];
  ...
  // time loop
  RF T = ptree.get<double>("problem.T");
  RF dt = ptree.get<double>("method.dt");
  RF timestepmax =  ptree.get<double>("method.dtmax");
  ...
  while (time<T-1e-8)
    {
      ...
      timestep = std::min(timestep,model.days()-1);
      int year = year_from_day(timestep);
      year = std::min(year,model.years()-1);
      for (size_t i=0; i<model.snowcover(0).data().size(); i++)
      ...
      for (size_t i=0; i<model.glacier(0).data().size(); i++)
        Dune::PDELab::Backend::native(glacier_vec)[i][0] = model.glacier(year).data()[i];
  ...
  for (const auto& e : elements(gv))
    {
      ...
      int cellx = std::floor(x[0]/90.0);
      int celly = std::floor(x[1]/90.0);
      Dune::PDELab::Backend::native(glacier_vec)[indexset.index(e)][0] = model.glacier(0)(cellx,celly);
    }
      ...
      for (const auto& e : elements(gv))
        {
          ...
          Dune::PDELab::Backend::native(glacier_vec)[index][0] = model.glacier(year)(cellx,celly);
        }
...
public:
  ...
  void run (Point p, std::string tag) const
  {
    // check correct number of parameters
    if (p.size()!=param_name.size())
      {
        std::cout << "Simulation.run: wrong number of parameters" << std::endl;
        return;
      }
    ...
    ptreeparser.readINITree("stok_long_term.ini",ptree);
