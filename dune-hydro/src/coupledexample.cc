...
/*************************************************/
// Example for coupled surface and subsurface flow
/*************************************************/
...
#include<dune/hydro/nonlineardiffusioncoupledmodel.hh>
...
public:
  ...
  CoupledModel (Dune::ParameterTree& _ptree) : ptree(_ptree), eps(1e-3)
  {
    ...
    rain = ptree.get("problem.rain",(Number)0.0);
    permeability = ptree.get("problem.permeability",(Number)1.0);
    
    L_i = ptree.get("problem.L_i",(Number)8e-5);
    L_e = ptree.get("problem.L_e",(Number)8e-5);
    C = ptree.get("problem.C",(Number)1.0);
  }
  ...
  Number bathymmetry_surface (const E& e, const X& xlocal) const
  {
    ...
    return 5.0 + x[0]*0.025 + std::min( (x[1]-25.0)*(x[1]-25.0)/625.0*50.0 , 5+std::abs(x[1]-25.0)/50.0 );
  }
  ...
  Number g_surface (const E& e, const X& x) const
  {
    ...
    return std::max(6.0,bathymmetry_surface(e,x) + std::max(5.0*(1.0-distance/15.0),0.0));
  }
  ...
  {
    Number firstterm  = L_i*(std::max(u_surface-bath_surf,0.0)/(C + std::max(u_surface-bath_surf,0.)))*std::max(u_surface-u_groundwater,0.);
    Number secondterm = L_e*std::max(u_groundwater - u_surface, 0.);
...
//********************************************************************************
// driver function solving the scalar problem
//********************************************************************************
  ...
  while (time<T-1e-8)
    {
      ...
      // increase timestep
      if (step-decreased_step>8 && step-increased_step>8)
        {
...
//********************************************************************************
// driver function solving the coupled problem
//********************************************************************************

template<typename GV, typename FEM>
void driverFV_coupled (const GV& gv, const FEM& fem, Dune::ParameterTree& ptree)
{
  // dimension and important types
  const int dim = GV::dimension;
  typedef double RF;                   // type for computations
  ...
  gfs.child(_0).name("surface");
  gfs.child(_1).name("groundwater");
  ...
  using USURFSUB = Dune::PDELab::GridFunctionSubSpace<GFS,Path0>;
  USURFSUB usurfsub(gfs);
  using UGROUNDSUB = Dune::PDELab::GridFunctionSubSpace<GFS,Path1>;
  UGROUNDSUB ugroundsub(gfs);

  // Make discrete grid functions for components
  using USURFDGF = Dune::PDELab::DiscreteGridFunction<USURFSUB,Z>;
  USURFDGF usurfdgf(usurfsub, z);
  using UGROUNDDGF = Dune::PDELab::DiscreteGridFunction<UGROUNDSUB, Z>;
  UGROUNDDGF ugrounddgf(ugroundsub, z); 
  ...
  using G = Dune::PDELab::CompositeConstraintsParameters<decltype(g_surface),decltype(g_groundwater)>;
  G g(g_surface, g_groundwater);
  ...
  auto bathymmetrylambda = [&](const auto& e, const auto& x){
    ...
    rv[0] = model.bathymmetry_surface(e,x);
    rv[1] = model.bathymmetry_groundwater(e,x);
    return rv;
  };
  auto bathymmetrygf = Dune::PDELab::
    makeInstationaryGridFunctionFromCallable(gv,bathymmetrylambda,model);
  Z bathymmetry(gfs);
  Dune::PDELab::interpolate(bathymmetrygf,gfs,bathymmetry); 
  USURFDGF bdgf_surface(usurfsub,bathymmetry);
  UGROUNDDGF bdgf_groundwater(ugroundsub,bathymmetry);
  ...
  // initialize simulation time,  the coefficient vector
  Dune::PDELab::interpolate(g_surface,usurfsub,z);
  Dune::PDELab::interpolate(g_groundwater,ugroundsub,z);

  // Make instationary grid operator
  typedef NonlinearDiffusionCoupledFV<MODEL,GFS> LOP;
  LOP lop(model,gfs);
  typedef Dune::PDELab::ISTL::BCRSMatrixBackend<> MBE;
  MBE mbe(2*dim+1); // guess nonzeros per row
  typedef Dune::PDELab::GridOperator<GFS,GFS,LOP,MBE,RF,RF,RF,CC,CC> GO0;
  ...
  typedef FVTemporalCoupled<MODEL,GFS> TLOP;
  TLOP tlop(model,gfs);
  typedef Dune::PDELab::GridOperator<GFS,GFS,TLOP,MBE,RF,RF,RF,CC,CC> GO1;
  GO1 go1(gfs,cc,gfs,cc,tlop,mbe);
  ...
  Z znew(z);
  USURFDGF znewdgf_surface(usurfsub,znew);
  UGROUNDDGF znewdgf_groundwater(ugroundsub, znew);

  // residual
  Z r(z);
  r = 0.0;
  std::cout << std::setprecision(10) << 0.345 << std::endl;
  USURFDGF rdgf_surface(usurfsub,r);
  UGROUNDDGF rdgf_groundwater(ugroundsub,r);
  go0.residual(z,r);
  Dune::PDELab::set_constrained_dofs(cc,0.0,r);
  ...
  typedef Dune::VTKSequenceWriter<GV> VTKSEQUENCEWRITER;
  VTKSEQUENCEWRITER vtkSequenceWriter(std::make_shared<VTKWRITER>(vtkwriter),filename,filename,"");
  typedef Dune::PDELab::VTKGridFunctionAdapter<USURFDGF> VTKFSURF;
  typedef Dune::PDELab::VTKGridFunctionAdapter<UGROUNDDGF> VTKFGROUND;
  vtkSequenceWriter.addCellData(std::shared_ptr<VTKFSURF>(new VTKFSURF(znewdgf_surface,"solution_surface")));
  vtkSequenceWriter.addCellData(std::shared_ptr<VTKFGROUND>(new VTKFGROUND(znewdgf_groundwater,"solution_groundwater")));
  vtkSequenceWriter.addCellData(std::shared_ptr<VTKFSURF>(new VTKFSURF(bdgf_surface,"bathymmetry_surface")));
  vtkSequenceWriter.addCellData(std::shared_ptr<VTKFGROUND>(new VTKFGROUND(bdgf_groundwater,"bathymmetry_groundwater")));
  vtkSequenceWriter.addCellData(std::shared_ptr<VTKFSURF>(new VTKFSURF(rdgf_surface,"r_surface")));
  vtkSequenceWriter.addCellData(std::shared_ptr<VTKFGROUND>(new VTKFGROUND(rdgf_groundwater,"r_groundwater")));
  vtkSequenceWriter.write(time,Dune::VTK::appendedraw);
  ...
  while (time<T-1e-8)
    {
      ...
      // accepting time step
      std::cout << "ACCEPTDT " << time << " " << dt << std::endl;
      time+=dt;
      step++;
      z = znew;
      ...
      // increase timestep
      if (step-decreased_step>25 && step-increased_step>25)
        {
          double newdt = std::min(timestepmax,std::sqrt(2.0)*dt);
          if (newdt>dt)
            {
...
int main(int argc, char** argv)
{
  try{
    ...
    ptreeparser.readINITree("coupledexample.ini",ptree);
