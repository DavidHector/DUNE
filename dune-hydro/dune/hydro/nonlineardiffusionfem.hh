...
#ifndef DUNE_HYDRO_LOCALOPERATOR_NONLINEARDIFFUSIONFEM_HH
#define DUNE_HYDRO_LOCALOPERATOR_NONLINEARDIFFUSIONFEM_HH
...
/** a local operator for spatial part of the nonlinear diffusion equation with conforming FEM
 *
 * \f{align*}{
...
class NonlinearDiffusionFEM :
  public Dune::PDELab::
    NumericalJacobianVolume<NonlinearDiffusionFEM<Param,FEM> >,
  public Dune::PDELab::
    NumericalJacobianApplyVolume<NonlinearDiffusionFEM<Param,FEM> >,
...
public:
  ...
  NonlinearDiffusionFEM (Param& param_, int incrementorder_=0)
    ...
    for (const auto& ip : rule)
      {
        ...
        auto F = param.F(eg.entity(),ip.position(),u,gradu);
	
        // integrate F(...)*grad phi_i
        auto factor = -1.0*ip.weight()*
          geo.integrationElement(ip.position());
        for (size_t i=0; i<lfsu.size(); i++)
          r.accumulate(lfsu,i,(F*gradphi[i][0])*factor);
      }
  }
};
 ...
 *
 */
template<typename Param, typename FEM, typename GFS, typename Z>
class NonlinearDiffusionFEMLinearized :
  public Dune::PDELab::
  NumericalJacobianVolume<NonlinearDiffusionFEMLinearized<Param,FEM,GFS,Z> >,
  public Dune::PDELab::
    NumericalJacobianApplyVolume<NonlinearDiffusionFEMLinearized<Param,FEM,GFS,Z> >,
  public Dune::PDELab::InstationaryLocalOperatorDefaultMethods<double>,
  public Dune::PDELab::FullVolumePattern,
  public Dune::PDELab::LocalOperatorDefaultFlags
{
  ...
  std::shared_ptr<Z> linearizationpoint;
...
public:
  ...
  //! constructor stores a copy of the parameter object
  NonlinearDiffusionFEMLinearized (Param& param_,  const GFS& gfs_, Z& z_, int incrementorder_=0)
    : param(param_),
      incrementorder(incrementorder_),
      pgfs(stackobject_to_shared_ptr(gfs_)),
      linearizationpoint(stackobject_to_shared_ptr(z_)),
      lfs(pgfs),
      lfs_cache(lfs)
  {}
                     ...
                     const LFSV& lfsv, R& r) const
  {
    ...
    // prepare access to outside linearization point
    typename Z::template LocalView<LFSCache> p_view(*linearizationpoint);
    lfs.bind(eg.entity());
    lfs_cache.update();
    std::vector<RF> zl(lfs.size());
    ...
    // evaluate at center
    RF z=0.0;
    Dune::FieldVector<RF,dim> gradz(0.0);
    auto ref_el = referenceElement(geo);
    auto localcenter = ref_el.position(0,0);
    {

      // evaluate basis functions
      auto& phihat = cache.evaluateFunction(localcenter,lfsu.finiteElement().localBasis());
      for (size_t i=0; i<lfsu.size(); i++)
        z += zl[i]*phihat[i];

      // evaluate gradient of shape functions
      auto& gradphihat = cache.evaluateJacobian(localcenter,lfsu.finiteElement().localBasis());

      // transform gradients of shape functions to real element
      const auto S = geo.jacobianInverseTransposed(localcenter);
      auto gradphi = makeJacobianContainer(lfsu);
      for (size_t i=0; i<lfsu.size(); i++)
        S.mv(gradphihat[i][0],gradphi[i][0]);

      for (size_t i=0; i<lfsu.size(); i++)
        gradz.axpy(zl[i],gradphi[i][0]);
    }
    ...
    for (const auto& ip : rule)
      {
        ...
        // compute gradient of u and u
        Dune::FieldVector<RF,dim> gradu(0.0);
        for (size_t i=0; i<lfsu.size(); i++)
          gradu.axpy(x(lfsu,i),gradphi[i][0]);

        // evaluate flux function
        auto F = param.F(eg.entity(),localcenter,u,z,gradu,gradz);
    ...
    for (const auto& ip : rule)
      {
        ...
        for (size_t i=0; i<lfsv.size(); i++)
          r.accumulate(lfsv,i,x(lfsu,i)*phihat[i]*factor);
      }
  }
    ...
    for (const auto& ip : rule)
      {
        ...
        RF factor = ip.weight() * geo.integrationElement(ip.position());
        for (size_t i=0; i<lfsv.size(); i++)
          mat.accumulate(lfsv,i,lfsu,i,phihat[i]*factor);
      }
  }
