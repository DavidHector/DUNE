#ifndef Udune_ftworth_snakeandislands_HH
#define Udune_ftworth_snakeandislands_HH
...
public:
  ...
  SnakeAndIslands(Dune::ParameterTree& ptree) {}
  ...
  A (const typename Traits::ElementType& e, const typename Traits::DomainType& x) const
  {
    ...
    // horizontal stripes for snake
    if ( xglobal[1] < 0.95*l1 && xglobal[1]> 0.90*l1 && xglobal[0] < 0.95*l0) {
        I[0][0] = contrast;
        I[1][1] = contrast;
        return I;         
    }

    if ( xglobal[1] < 0.55*l1 && xglobal[1]> 0.50*l1 && xglobal[0] > 0.05*l0 && xglobal[0] < 0.95*l0) {
        I[0][0] = contrast;
        I[1][1] = contrast;
        return I;         
    }

    if ( xglobal[1] < 0.05*l1 && xglobal[1]> 0.00*l1 && xglobal[0] > 0.05*l0 && xglobal[0] < 0.95*l0) {
        I[0][0] = contrast;
        I[1][1] = contrast;
        return I;         
    }

    // vertical stripes for snake
    if ( xglobal[1] < 0.55*l1 && xglobal[1]> 0.00*l1 && xglobal[0] > 0.05*l0 && xglobal[0] < 0.05*l0+0.05) {
        I[0][0] = contrast;
        I[1][1] = contrast;
        return I;         
    }   
    if ( xglobal[1] < 0.95*l1 && xglobal[1]> 0.50*l1 && xglobal[0] > 0.95*l0-0.05 && xglobal[0] < 0.95*l0) {
        I[0][0] = contrast;
        I[1][1] = contrast;
        return I;         
    }
    ...
    // fat islands
    for (int i = 1; i < 9; i++){
      double leftboundary = double(i);
      if (xglobal[1] < 0.30*l1 && xglobal[1]> 0.20*l1 && xglobal[0] < leftboundary + l0*0.1 && xglobal[0]> leftboundary) {
        I[0][0] = contrast;
        I[1][1] = contrast;
        return I;  
      }
