...
public:
  ...
  SPE10Problem (Dune::ParameterTree& ptree_) : ptree(ptree_), kx(N), ky(N), kz(N)
  {
    ...
    auto extension = ptree.get<std::vector<double>>("grid.extension");
    LX = extension[0];
    LY = extension[1];
    LZ = extension[2];
    auto cells = ptree.get<std::vector<int>>("grid.cells");
    NX = cells[0];
    NY = cells[1];
    NZ = cells[2];
    hx = LX/nx;
    ...
    // read SPE 10 data file and check parameters
    // if (LX!=1200.0) DUNE_THROW(Dune::Exception,"spe10: LX must be 1200.0");
    // if (LY!=2200.0) DUNE_THROW(Dune::Exception,"spe10: LY must be 2200.0");
    // if (LZ!=170.0) DUNE_THROW(Dune::Exception,"spe10: LZ must be 170.0");
    // if (NX!=nx) DUNE_THROW(Dune::Exception,"spe10: NX must be nx");
    // if (NY!=ny) DUNE_THROW(Dune::Exception,"spe10: NY must be ny");
    // if (NZ!=nz) DUNE_THROW(Dune::Exception,"spe10: NZ must be nz");
    ...
    std::cout << "kxmin=" << min << " kxmax=" << max << std::endl;
    ...
    std::cout << "kymin=" << min << " kymax=" << max << std::endl;
    ...
    std::cout << "kzmin=" << min << " kzmax=" << max << std::endl;
  ...
  A (const typename Traits::ElementType& e, const typename Traits::DomainType& x) const
  {
    ...
    I[0][0] = kxmean*(1.0-scaling) + scaling*kx[k*nx*ny+j*nx+i];
    I[1][1] = kymean*(1.0-scaling) + scaling*ky[k*nx*ny+j*nx+i];
    I[2][2] = kzmean*(1.0-scaling) + scaling*kz[k*nx*ny+j*nx+i];
  ...
  bctype (const typename Traits::IntersectionType& is, const typename Traits::IntersectionDomainType& x) const
  {
    ...
    if (xglobal[1]>LY-1e-6) return Dune::PDELab::ConvectionDiffusionBoundaryConditions::Dirichlet;
