...
int main(int argc, char** argv)
{
  try{
    ...
    // vtkwriter.addCellData(prec.get_partition(2),"partition 2");
    ...
    //   for (int p=0; p<std::min((size_t)1024,prec.number_of_subdomains(level)); p++)
    //     for (int i=0; i<std::min((size_t)20,prec.number_of_kerAB_vectors(level,p)); i++)
    //       {
    //         std::vector<size_t> v; v.push_back(level);  v.push_back(p);  v.push_back(i); 
    //         viz_vectors.push_back(v);
    //       }
      ...
      for (int i=0; i<n_bv; i++)
        {
          ...
          Dune::PDELab::Backend::native(*basisvectors[i]) = prec.prolongated_kerAB_vector(l,p,k); // fill value
          basisvectordgfs[i] = std::shared_ptr<DGF>( new DGF(*disc.gfs,*basisvectors[i]) );
          std::ostringstream s1;
          s1 << "kerABvector_" << l << "_" << p << "_" << k;
