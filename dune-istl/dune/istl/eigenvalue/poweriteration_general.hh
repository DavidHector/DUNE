...
namespace Dune
{
  ...
  */
  ...
  public:
    ...
    /**
     * \brief Perform the general inverse iteration with shift algorithm to compute an
     ...
     *                             solver. Defaults to false.
     * 
     * \param[in]     b_      The right-hand side matrix of the problem.
    ...
    template <typename ISTLLinearSolver,
              bool avoidLinSolverCrime = false,
              typename RighthandSideSolver = ISTLLinearSolver>
    inline void applyGeneralInverseIteration (const BCRSMatrix& b_,
                                              const Real& gamma,
                                              const Real& epsilon,
                                              ISTLLinearSolver& solver,
                                              RighthandSideSolver& rightSolver,
                                              BlockVector& x, Real& lambda) const
    {
      // print verbosity information
      if (verbosity_level_ > 0)
      ...
      // Create matrix A = (m_ - gamma*b_) and give it to the solver
      // The eigenvalue now looks like A^(-1) b_ * x = 1/lambda x
      // TODO: Create function similar to updateShiftMu(gamma, solver) because we also need the new iteration matrix
      BCRSMatrix A(m_);
      A.axpy(-gamma, b_);
      SolverHelper<ISTLLinearSolver, BCRSMatrix>::setMatrix (solver, A); // maybe create a general updateShiftMu function to avoid this when nothing is changed
      SolverHelper<RighthandSideSolver, BCRSMatrix>::setMatrix (rightSolver, b_);
      // new x = b: Ab=Bx
      ...
      Blockvector temprightside(x);
      b_.mv(x, rightside);
      BlockVector templeftside(x);

      // Problem: I have to recalculate the rightside on each iteration
      while (r_norm > epsilon)
      {
        // update and check number of iterations
        if (++nIterations_ > nIterationsMax_)
        ...
        // part 1: solve A * y = b_ * x for y
        // (protect x from being changed)
        temp = x;
        solver.apply(y,temprightside,solver_statistics); // y = A^(-1) b_ x
        ...
        if (avoidLinSolverCrime)
        {
          ...
          // First get new right side
          b_.mv(y, temprightside); // temprightside = b_ * y  
          m_.mv(y, templeftside); // m_ * y // not for the inverse problem, but for the normal one
          // Generalised Rayleigh factor: (x*m_*x)/(x*b_*x)
          // Check out the exploitation of the norm in the original file, something is funky here
          lambda = (y * templeftside) / (y * temprightside);
        ...
        else
        {
          ...
          // TODO: Look at generalised Rayleigh factor
