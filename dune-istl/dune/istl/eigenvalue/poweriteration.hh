  ...
  */
  ...
  public:
                                              ...
                                              BlockVector& x, Real& lambda) const
    {
      ...
      updateShiftMuGeneral(gamma, b_, solver);
      ...
      while (r_norm > epsilon)
      {
        ...
        // (protect x from being changed)
        temp = x;
        solver.apply(y,rightside,solver_statistics); // y = A^(-1) b_ x
        ...
        if (avoidLinSolverCrime)
        {
          ...
          b_.mv(y, rightside); // rightside = b_ * y  
          ...
          lambda = (y * templeftside) / (y * rightside);
  ...
  protected:
    ...
    /**
     * \brief Update shift mu_, i.e. update the generalised iteration operator/matrix
     *        (m_ - mu_*b_).
     *
     * \note Does nothing if new shift equals the old one.
     *
     ...
     *
     * \param[in] mu     The new shift.
     * \param[in] b_     The shift matrix.
     * \param[in] solver The DUNE-ISTL InverseOperator which is used
     *                   as a linear solver.
     *
     */
    template <typename ISTLLinearSolver>
    inline void updateShiftMuGeneral (const Real& mu,
                                      const BCRSMatrix& b_,
                                      ISTLLinearSolver& solver) const
    {
      // do nothing if new shift equals the old one
      ...
      if (itMatrix_)
      {
        constexpr Real zero = 0.0; // Not sure if this indirection is necessary
        *itMatrix_ *= zero; // empty matrix
        itMatrix_->axpy(-mu_, b_); // itMatrix = m_ - mu_*b_
        // notify linear solver about change of the iteration matrix object
        SolverHelper<ISTLLinearSolver,BCRSMatrix>::setMatrix
          (solver,*itMatrix_);
      }
