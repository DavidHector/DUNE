...
int main(int argc, char** argv)
{
    try
    {
        ...
        auto N  = Dune::filledArray<dim, unsigned int>(3);
        ...
        for (const auto& e : elements(gridview))
        {
            ...
            // *** ADD CODE HERE (1/4) ***
            std::cout << std::endl;
            std::cout << "Cell Center: " << geo.center() << std::endl;
            auto corner_sum = geo.corner(0);
            for (size_t i = 1; i < geo.corners(); i++)
            {
                corner_sum += geo.corner(i);
            }

            std::cout << "Calculated Cell center = " << corner_sum[0]/geo.corners() << " " << corner_sum[1]/geo.corners() << std::endl;
            ...
            for (const auto& is : intersections(gridview, e))
            {
                ...
                // *** ADD CODE HERE (2/4) ***
                for (size_t i = 0; i < fgeo.corners(); i++)
                {
                    std::cout << "(" << fgeo.corner(i) << "), ";
                }
                std::cout << "center: " << fgeo.center() << std::endl;
